...
$(function() {
	...
	jQuery.validator.addMethod("isMobile", function(value, element) {
		...
		return this.optional(element) || (length == 11 &&
			/^(((13[0-9]{1})|(15[0-35-9]{1})|(17[0-9]{1})|(18[0-9]{1}))+\d{8})$/.test(value));
	}, "手机号码格式不正确。");
	...
	// 验证视频上传类型
	/* jQuery.validator.addMethod("isVideo", function(abc, element,value) {
		...
		var name = file.name;
		var fileType = name.substr(name.lastIndexOf('.'),name.length);
		var acceptType = ['.mp4','.ogg','avi','wmv','mpeg'];
		for(var type of acceptType){
			if(type == fileType.toLowerCase()){
				console.log(2);
